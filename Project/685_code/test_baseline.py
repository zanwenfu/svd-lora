...
def parse_args():
    parser = argparse.ArgumentParser(description="测试原始DistilBERT模型")
    parser.add_argument("--dataset", type=str, default="sst2",
                        choices=["sst2", "imdb"],
                        help="数据集选择：sst2或imdb")
    parser.add_argument("--model_name", type=str, default="distilbert-base-uncased",
                        help="模型名称（默认: distilbert-base-uncased）")
...
def main():
    ...
    # 加载原始的未微调模型
    print(f"\n正在加载原始模型: {args.model_name}")
    print("注意：这是未经微调的预训练模型")
    ...
    if args.dataset == "sst2":
        ...
        eval_name = "验证集"
        print(f"注意：GLUE的SST-2测试集标签被隐藏，使用验证集评估")
    else:  # imdb
        eval_dataset = tokenized_dataset["test"]
        eval_name = "测试集"
    
    print(f"{eval_name}样本数: {len(eval_dataset)}")
    ...
    # 在评估集上评估
    print("\n" + "="*60)
    print(f"开始评估原始模型: {args.model_name} on {args.dataset.upper()}")
    print("="*60 + "\n")
    ...
    # 打印结果
    print("\n" + "="*60)
    print(f"原始模型{eval_name}评估结果 ({args.model_name} on {args.dataset.upper()})")
    print("="*60)
    print(f"\n整体指标:")
    print(f"  准确率 (Accuracy):     {metrics['eval_accuracy']:.4f} ({metrics['eval_accuracy']*100:.2f}%)")
    ...
    print(f"模型: {args.model_name} (原始未微调)")
    print(f"{eval_name}准确率: {metrics['eval_accuracy']*100:.2f}%")
    print(f"这是baseline性能，LoRA微调后应该能显著提升")
    print("="*60 + "\n")
